...
http_error_messages = {
    401: "ERROR: There was a problem during authentication.\nDouble check "
         "that your username and password are correct, and that you have "
         "permission to read from or write to the specified repositories.",
    404: "ERROR: Unable to find the specified repository.\nDouble check the "
         "spelling for the source and target repositories. If either "
         "repository is private, make sure the specified user is allowed "
         "access to it."
}
...
def init_config():
    ...
    config_group.add_argument('--config',
                              help="The location of the config file (either absolute, or relative "
    ...
    config_group.add_argument('--no-config', dest='no_config',
                              action='store_true',
                              help="No config file will be used, and the default `config.ini` will "
                                   "be ignored. Instead, all settings are either passed as "
                                   "arguments, or (where possible) requested from the user as a "
                                   "prompt.")

    arg_parser.add_argument('-u', '--username',
                            help="The username of the account that will create the new issues. "
                                 "The username will not be stored anywhere if passed in as an "
                                 "argument.")
    arg_parser.add_argument('-p', '--password',
                            help="The password (in plaintext) of the account that will create the "
                                 "new issues. The password will not be stored anywhere if passed "
                                 "in as an argument.")
    arg_parser.add_argument('-s', '--source',
                            help="The source repository which the issues should be copied from. "
                                 "Should be in the format `user/repository`.")
    arg_parser.add_argument('-t', '--target',
                            help="The destination repository which the issues should be copied "
                                 "to. Should be in the format `user/repository`.")
    ...
    def load_config_file(cfg_file_name):
        try:
    ...
    if args.no_config:
        print(
            "Ignoring default config file. You may be prompted for some "
            "missing settings.")
    elif args.config:
        ...
        if load_config_file(config_file_name):
            ...
            sys.exit(
                "ERROR: Unable to find or open config file '%s'" %
                config_file_name)
    else:
        config_file_name = default_config_file
        if load_config_file(config_file_name):
            print(
                "Loaded options from default config file in '%s'" %
                config_file_name)
        else:
    ...
    if not config.has_option('source', 'repository'):
        sys.exit(
            "ERROR: There is no source repository specified either in the "
            "config file, or as an argument.")
    if not config.has_option('target', 'repository'):
        sys.exit(
            "ERROR: There is no target repository specified either in the "
            "config file, or as an argument.")
    ...
    def get_server_for(which):
        ...
        config.set(which, 'url',
                   "%s/repos/%s" % (api_url, config.get(which, 'repository')))

    get_server_for('source')
    get_server_for('target')

    # Prompt for username/password if none is provided in either the config
    # or an argument
        ...
        if not config.has_option(which, 'username'):
            if config.has_option('login', 'username'):
                config.set(which, 'username', config.get('login', 'username'))
                    ...
                    "Do you wish to use the same credentials for the target "
                    "repository?")):
...
def get_issues_by_id(which, issue_identifiers):
    # Populate issues based on issue IDs
    i = []
    for issue_id in issue_identifiers:
        i.append(get_issue_by_id(which, int(issue_id)))
...
def import_milestone(source):
    ...
    result_milestone = send_request('target', "milestones", {k: v for k, v in data.items() if v is not None})
...
def import_label(source):
    ...
    result_label = send_request('target', "labels", {k: v for k, v in data.items() if v is not None})
...
def import_issues(issue_list):
    ...
    for issue in issue_list:
        ...
        if (config.getboolean('settings', 'import-labels')
                and 'labels' in issue
                and issue['labels'] is not None):
    ...
    print("You are about to add to '" + config.get('target',
                                                   'repository') + "':")
